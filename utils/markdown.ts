...
import { escape as escapeHtml } from "$std/html/mod.ts";
...
class DefaultRenderer extends Marked.Renderer {
  ...
  override code({ lang: info, text }: Marked.Tokens.Code): string {
    ...
    if (grammar === undefined) {
      out += `<pre><code class="notranslate">${escapeHtml(text)}</code>`;
    } else {
      const html = Prism.highlight(text, grammar, lang);
      out +=
        `<pre className="highlight highlight-source-${lang} notranslate lang-${lang}"><code>${html}</code>`;
    }

    out += `
    <button className="absolute top-0 right-10">Copy</button>
    </pre></div>
  `;
